...
import { Popover, PopoverContent, PopoverTrigger } from '@contecon/ui/components/popover'
...
export function SchedulesInboxCard({ name, description, startDate, endDate, identifierColor = '#fff' }: InboxItem) {
      ...
      <PopoverTrigger asChild>
        <button className='flex flex-col items-start cursor-pointer justify-center py-4 px-0 bg-card/80 hover:bg-card/100 transition-all duration-300 rounded-lg shadow-sm border border-border'>
          <div className='px-5 flex items-center gap-2'>
            <div className='size-2 rounded-full' style={{ backgroundColor: `${identifierColor}` }} />
        ...
        <div className='flex flex-col gap-2'>
          <div className='flex items-start justify-between pr-3'>
            <div className='flex flex-col gap-2 px-4 mt-auto'>
              ...
              <span className='text-xxs font-normal text-muted-foreground line-clamp-2'>{description}</span>
            </div>
          ...
          <div className='px-3 mt-auto flex items-center gap-2'>
            <Badge variant='schedule' className='text-xxs font-semibold text-center'>
