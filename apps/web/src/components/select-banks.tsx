...
export function SelectBanks({ register, setFormValue, name, banks = [], isLoading = false, defaultValue }: Props) {
  if (!isLoading && (!banks || banks.length === 0)) {
    return <Input placeholder='Digite o nome do banco' {...register} name={name} />
  }

  return <SelectBanksClient banks={banks} setFormValue={setFormValue} defaultValue={defaultValue} />
}
